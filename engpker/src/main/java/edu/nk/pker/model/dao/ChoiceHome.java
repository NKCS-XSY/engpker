...
import edu.nk.pker.model.po.Choice;

/**
 * Home object for domain model class Choice.
 * @see edu.nk.pker.model.dao.Choice
 * @author Hibernate Tools
 */
public class ChoiceHome {

	private static final Log log = LogFactory.getLog(ChoiceHome.class);
	...
	public void persist(Choice transientInstance) {
		log.debug("persisting Choice instance");
		try {
	...
	public Choice merge(Choice detachedInstance) {
		...
		try {
			Choice result = (Choice) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Choice findById(java.lang.Integer id) {
		...
		try {
			Choice instance = (Choice) sessionFactory.getCurrentSession().get(
					"edu.nk.pker.model.dao.Choice", id);
			if (instance == null) {
	...
	public List findByExample(Choice instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("edu.nk.pker.model.dao.Choice")
