...
import edu.nk.pker.model.po.Note;

/**
 * Home object for domain model class Note.
 * @see edu.nk.pker.model.dao.Note
...
public class NoteHome {
	...
	public Note merge(Note detachedInstance) {
		...
		try {
			Note result = (Note) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Note findById(java.lang.Integer id) {
		...
		try {
			Note instance = (Note) sessionFactory.getCurrentSession().get(
					"edu.nk.pker.model.dao.Note", id);
			if (instance == null) {
	...
	public List findByExample(Note instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("edu.nk.pker.model.dao.Note")
